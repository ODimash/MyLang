...
struct Flags
{
	string myPath;					// path to the MyLang exeutable that is now running
...
int main(int argc, char ** argv)
{
	...
	if (flags.flagError)
	{
		cout << "try 'MyLang -h' for help" << endl;
		return 0;
	}

	if (flags.help)
	{
		cout << "MyLang v" << VERSION_X << "." << VERSION_Y << "." << VERSION_Z << endl;
		cout << "usage: MyLang [options] [source file] [options]" << endl;
		cout << "options: " << endl;
		cout << "-v, -version      display the version of MyLang" << endl;
	...
	if (flags.version)
	{
		cout << "MyLang version " << VERSION_X << "." << VERSION_Y << "." << VERSION_Z << endl;
		return 0;
	}

	MyLangProgram program;
	...
	{
		cout << "multiple source files specified, MyLang does not currently support this" << endl;
		cout << "try 'MyLang -h' for help" << endl;
		return 0;
	}
		...
		else
		{
			...
			if (!flags.binOutFile.empty() || flags.runCompiled)
			{
				...
				if (flags.runCompiled)
				{
					...
					cmd = "./"+binFileName + " --running-from-MyLang " + str::join(cmdLineArgs, " ", false);
