...
ActionData::ActionData(Type returnTypeIn, Type inLeftTypeIn, Type inRightTypeIn)
{
	...
	{
		throw MyLangError("ActionData created with null type", INTERNAL_ERROR);
	}
}
...
public:
			...
			: ActionData(returnTypeIn, inLeftTypeIn, inRightTypeIn)
	{
		...
		if (lambdaIn==nullptr)
		{
			...
			{
				throw MyLangError("action '"+textIn+"' has not yet been implemented for the interpreter", RUNTIME_ERROR);
			};
		}
		else
		{
