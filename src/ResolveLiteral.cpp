...
Action resolveLiteral(Token token)
{
	...
	{
		throw MyLangError(FUNC+" called on token that is not a literal", INTERNAL_ERROR, token);
	}
	...
	{
		throw MyLangError("tried to make literal with invalid type of " + type->getString(), INTERNAL_ERROR, token);
	}
}
