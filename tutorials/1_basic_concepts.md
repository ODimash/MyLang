...
The tutorials for most programming languages start with a hello world program, but MyLang isn't most programming languages. Instead, lets start with a program that prints out something far more important.
...
A __Bool__ can only be `True` or `False`. If you think it should be `true` and `false` instead, you can email your complaints to williamwold@idontgiveafuck.com.
...
In general, operators in MyLang work the same as in any other language. It has all the ones you would expect with sensible order of operations. The following are the only major differences between operators in MyLang and C-style langauges:
...
* The short circuit aspect of boolean operators doesn't work yet, so `False && functionReturningBool` will execute functionReturningBool. This will be fixed soon.
...
Comments are parts of your program that the compiler doesn't look at, so you can write notes and whatnot. In MyLang, single-line comments start with a `#`. Multi-line comments start with `//` and end with `\\`.
